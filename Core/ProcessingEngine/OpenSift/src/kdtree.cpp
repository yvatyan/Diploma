		    ...
		    struct feature*** nbrs, int max_nn_chks )
{
  ...
  _nbrs = static_cast<feature**>(calloc( k, sizeof( struct feature* ) ));
  min_pq = minpq_init();
  ...
  while( min_pq->n > 0  &&  t < max_nn_chks )
      ...
      for( i = 0; i < expl->n; i++ )
	{
	  ...
	  bbf_data = static_cast<struct bbf_data*>(malloc( sizeof( struct bbf_data ) ));
	  if( ! bbf_data )
	    {
  ...
  for( i = 0; i < n; i++ )
    {
      bbf_data = static_cast<struct bbf_data*>(_nbrs[i]->feature_data);
			    ...
			    CvRect rect, int model )
{
  ...
  sp_nbrs = static_cast<feature**>(calloc( k, sizeof( struct feature* ) ));
  for( i = 0; i < n; i++ )
    {
      if( model )
...
static struct kd_node* kd_node_init( struct feature* features, int n )
{
  ...
  kd_node = static_cast<struct kd_node*>(malloc( sizeof( struct kd_node ) ));
...
static void assign_part_key( struct kd_node* kd_node )
{
  ...
  tmp = static_cast<double*>(calloc( n, sizeof( double ) ));
  for( i = 0; i < n; i++ )
...
static double rank_select( double* array, int n, int r )
{
  ...
  tmp = static_cast<double*>(calloc( gr_tot, sizeof( double ) ));
