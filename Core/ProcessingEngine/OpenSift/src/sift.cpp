...
static IplImage* create_init_img( const IplImage*, int, double );
static IplImage* convert_to_gray32( const IplImage* );
...
*/
static IplImage* create_init_img( const IplImage* img, int img_dbl, double sigma )
{
  IplImage* gray, * dbl;
  double sig_diff;
...
*/
static IplImage* convert_to_gray32( const IplImage* img )
{
  IplImage* gray8, * gray32;
  
  gray32 = cvCreateImage( cvGetSize(img), IPL_DEPTH_32F, 1 );
  if( img->nChannels == 1 )
    gray8 = static_cast<IplImage*>(cvClone( img ));
  else
    {
...
static struct feature* new_feature( void )
{
  ...
  ddata = static_cast<detection_data*>(malloc( sizeof( struct detection_data ) ));
