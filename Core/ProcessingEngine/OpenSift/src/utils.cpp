...
char* replace_extension( const char* file, const char* extn )
{
  ...
  new_file = static_cast<char*>(calloc( strlen( file ) + strlen( extn ) + 2,  sizeof( char ) ));
...
char* prepend_path( const char* path, const char* file )
{
  ...
  char* pathname = static_cast<char*>(calloc( n, sizeof(char) ));
...
*/
void display_big_img( IplImage* img, const char* title )
{
  IplImage* small;
  GdkScreen* scr;
