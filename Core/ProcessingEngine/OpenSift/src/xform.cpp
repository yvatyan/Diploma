				 ...
				 struct feature*** matched )
{
  ...
  _matched = static_cast<feature**>(calloc( n, sizeof( struct feature* ) ));
  for( i = 0; i < n; i++ )
    if( get_match( features + i, mtype ) )
      {
	rdata = static_cast<ransac_data*>(malloc( sizeof( struct ransac_data ) ));
					    ...
					    int m )
{
  ...
  sample = static_cast<feature**>(calloc( m, sizeof( struct feature* ) ));
  for( i = 0; i < m; i++ )
    {
      do
	{
				 ...
				 CvPoint2D64f** pts, CvPoint2D64f** mpts )
{
  ...
  _pts = static_cast<CvPoint2D64f*>(calloc( n, sizeof( CvPoint2D64f ) ));
  _mpts = static_cast<CvPoint2D64f*>(calloc( n, sizeof( CvPoint2D64f ) ));
			   ...
			   struct feature*** consensus )
{
  ...
  _consensus = static_cast<feature**>(calloc( n, sizeof( struct feature* ) ));
